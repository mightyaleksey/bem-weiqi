...
/* begin: ../desktop.blocks/scheme/scheme.js */
...
/* end: ../desktop.blocks/scheme/scheme.js */
/* begin: ../desktop.blocks/canvas/canvas.js */
modules.define('i-bem__dom', function (provide, dom) {
    'use strict';
...
/* end: ../desktop.blocks/canvas/canvas.js */
/* begin: ../desktop.blocks/board/board.vanilla.js */
modules.define('board', ['inherit', 'events', 'objects'], function (provide, inherit, events, objects) {
    'use strict';

    /**
     * Ходы, Точки
     *
     * В Го камень становится точкой. Ход и камень описываются одинаково: две строчные буквы.
     * Первая буква обозначает столбец (слева направо), вторая - строку (сверху вниз).
     * Верхняя левая часть доски используется для маленьких досок, например диапазон от 'a' до 'm' для доски 13х13.
     * Пропуск хода описывается как '[]' или '[tt]'. '[tt]' используется для совместимости со стандартом FF[3].
     * Использование строчных букв позволяет описать доску максимум 26х26.
     * В стандарте FF[4] можно описать доску 52х52. Заглавные буквы описывают точки от 27 до 52, т.е. 'a'=1 ... 'z'=26, 'A'=27 ... 'Z'=52
     */

    /**
     * Хелпер для работы с буквенными координатами
     */
    var Point = inherit({
        /**
        ...
        adjacent: function (point) {
            var points = [];
            var x = point.charAt(0);
            var y = point.charAt(1);

            this.next(x) && points.push(this.next(x) + y);
            this.next(y) && points.push(x + this.next(y));
            this.previous(x) && points.push(this.previous(x) + y);
            this.previous(y) && points.push(x + this.previous(y));
        ...
        getMap: function () {
            return this.map ||
                (this.map = Array.apply(null, new Array(this.size))
                    .map(function (u, i) {
        ...
        next: function (x) {
            var map = this.getMap();
            var ind = map.indexOf(x);
            return ind > -1 ? map.charAt(ind + 1) : '';
        },
    ...
    var Board = inherit(events.Emitter, {
         ...
         */
        __constructor: function (size) {
            // В качестве ключа выступают две строчные буквы,
            // которые описывают координаты точки.
            // В качестве значения выступает тип камня:
            // - 0 - черный,
            // - 1 - белый
            this.board = {};
            this.blackMove = true;
        ...
        /**
         * Когда Ч (соотв. Б) играет, ставится камень этого цвета на данной позиции (независимо от того, что было до).
         * Приложение должно проверить любые Б (соотв. Ч) группы, размещенные рядом с камнем. Если у них нет степеней свободы, они должны быть удалены и счет увеличен соответственно.
         * Далее, Ч (соотв. Б) группа, к которой принадлежит сыгранный камень должна быть проверена на количество степеней свободы и если их нет, то быть удалена и счет увеличен соответственно.
         * В конце, число ходов должно быть увеличено на единицу.
         * @param {String} [point]
         */
        play: function (point) {
            if (this.board.hasOwnProperty(point)) {
                return;
            }

            this.board[point] = this.blackMove ? 1 : 2;
            this.blackMove = !this.blackMove;
            this.move++;

            this.getAdjacentPoints(point)
                .concat(point)
                .forEach(function (point) {
                    var g = this.getGroup(point);
                    this.countLiberties(g) === 0 && this.removeGroup(g);
                }, this);

            this.board.hasOwnProperty(point) &&
                this.emit('add', {
                    color: this.board[point],
                    point: point,
                    x: point.charCodeAt(0) - 'a'.charCodeAt(0),
                    y: point.charCodeAt(1) - 'a'.charCodeAt(0)
                });
        },
        ...
        countLiberties: function (group) {
            ...
            Object.keys(group)
                .reduce(function (arr, point) {
                    return arr.concat(_this.getAdjacentPoints(point));
                }, [])
                .filter(function (p, i, arr) {
                    return arr.indexOf(p) === i;
                })
                .forEach(function (p) {
                    _this.board.hasOwnProperty(p) || liberties++;
                });
        ...
        /**
         * Отдает список соседних пересечений
         * @param {String} [point]
         * @returns {Array}
         */
        getAdjacentPoints: function (point) {
            return new Point(this.size).adjacent(point);
        },

        /**
         * Получить список точек, составляющих одну группу
         * @param {String} [point]
         * @returns {Object}
         */
        getGroup: function (point) {
            ...
            if (this.board.hasOwnProperty(point)) {
                ...
                while (queue.length > 0) {
                    var point = queue.pop();
                    group[point] = this.board[point];

                    this.getAdjacentPoints(point).forEach(function (point) {
                        this.board[point] === color &&
                            !group.hasOwnProperty(point) &&
                                queue.push(point);
                    }, this);
                }
            }
        ...
        removeGroup: function (group) {
            Object.keys(group).forEach(function (point) {
                this.emit('remove', {
                    color: group[point],
                    point: point,
                    x: point.charCodeAt(0) - 'a'.charCodeAt(0),
                ...
                delete this.board[point];

                this.captured[group[point] === 1 ? 'black' : 'white']++;
            }, this);
        }
    });
...
/* end: ../desktop.blocks/board/board.vanilla.js */
...
/* begin: ../desktop.blocks/scheme/_theme/scheme_theme_casual.js */
modules.define('i-bem__dom', function (provide, dom) {
    'use strict';
...
/* end: ../desktop.blocks/scheme/_theme/scheme_theme_casual.js */
/* begin: ../desktop.blocks/scheme/_type/scheme_type_game.js */
modules.define('i-bem__dom', ['board'], function (provide, board, dom) {
    ...
    dom.decl({
        ...
        handleBoardEvent: function (e, d) {
            switch(e.type) {
            case 'add':
                this.drawStone(d.x, d.y, d.color === 2 ? true : false);
                break;
            case 'remove':
...
/* end: ../desktop.blocks/scheme/_type/scheme_type_game.js */
/* begin: ../desktop.blocks/scheme/_marking/scheme_marking_yes.js */
modules.define('i-bem__dom', function (provide, dom) {
    'use strict';
...
/* end: ../desktop.blocks/scheme/_marking/scheme_marking_yes.js */
